...
// pattern pairs a compiled regex with its PII type, confidence, and optional validator.
// Confidence reflects how specifically the regex identifies the target PII type.
// The effectiveConfidence field includes the pack positional multiplier.
type pattern struct {
	re                  *regexp.Regexp
	piiType             PIIType
	confidence          float64
...
type Options struct {
	...
	PackDecayRate       float64                   // per-position confidence decay for packs
	PatternPacks        []config.PatternPack      // ordered list of packs with enable state
	CustomPIIPatterns   []config.CustomPIIPattern // user-defined patterns
}
...
func NewWithCacheAndCapacity(opts Options) *Anonymizer {
	...
	// Default pack config: all built-in packs enabled for backward compatibility.
	if len(opts.PatternPacks) == 0 {
		opts.PatternPacks = []config.PatternPack{
			{Pack: "GLOBAL", Enabled: true},
			{Pack: "DE", Enabled: true},
			{Pack: "SECRETS", Enabled: true},
			{Pack: "FR", Enabled: true},
			{Pack: "NL", Enabled: true},
			{Pack: "FINANCE_EU", Enabled: true},
			{Pack: "HEALTHCARE", Enabled: true},
			{Pack: "US", Enabled: true},
		}
	}
	...
	a.loadPacks(opts.PatternPacks, opts.PackDecayRate, opts.CustomPIIPatterns)
	return a
}
...
// loadPacks builds the pattern list from the pack registry.
// Only patterns whose pack appears as an enabled entry in packList are loaded.
// Pack position determines the likelihood multiplier applied to base confidence.
// Custom patterns are appended after built-in patterns for their pack.
func (a *Anonymizer) loadPacks(packList []config.PatternPack, decayRate float64, customs []config.CustomPIIPattern) {
	// Build enabled pack set with position (1-indexed among enabled packs).
	enabledPos := make(map[string]int)
	pos := 1
	for _, pp := range packList {
		if pp.Enabled {
			enabledPos[pp.Pack] = pos
			pos++
		}
	}

	// Load built-in patterns from the self-registered pack registry.
	allEntries := packs.All()
	for _, entry := range allEntries {
		p, ok := enabledPos[entry.Pack]
		if !ok {
			continue
		}
		multiplier := 1.0 - float64(p-1)*decayRate
		if multiplier < 0 {
			multiplier = 0
		}
		ec := entry.Confidence * multiplier
		if entry.Literal {
			ec = entry.Confidence // literals carry declared confidence without decay
		}
	...
	for _, cp := range customs {
		p, ok := enabledPos[cp.Pack]
		if !ok {
			log.Printf("[ANONYMIZER] Warning: custom pattern %q references unknown/disabled pack %q, skipping", cp.Type, cp.Pack)
			continue
		}
		var re *regexp.Regexp
		if cp.Literal {
			re = regexp.MustCompile(`(?i)` + regexp.QuoteMeta(cp.Pattern))
		} else {
			var err error
			re, err = regexp.Compile(cp.Pattern)
			if err != nil {
				log.Printf("[ANONYMIZER] Warning: custom pattern %q invalid regex: %v, skipping", cp.Type, err)
				continue
			}
		}
		multiplier := 1.0 - float64(p-1)*decayRate
		...
		ec := cp.Confidence * multiplier
		if cp.Literal {
			ec = cp.Confidence
		}
		a.patterns = append(a.patterns, pattern{
			re:                  re,
			piiType:             PIIType(strings.ToUpper(cp.Type)),
...
// If the pattern has a validator, validation failure drops the candidate.
...
func (a *Anonymizer) tokenForMatch(p pattern, match string) string {
	// Validation layer: if present and fails, drop candidate.
	if p.validate != nil && !p.validate(match) {
		log.Printf("[ANONYMIZER] validation failed for %s match, dropping candidate", p.piiType)
		return match // return original — not PII
	}

	if !a.useAI || p.effectiveConfidence >= a.aiThreshold {
...
// Tokens use [PII_<TYPE>_<16hex>] notation, e.g. [PII_EMAIL_c160f8cc2654c143].
//
