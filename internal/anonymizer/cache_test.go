...
func TestMemoryCacheBasicOperations(t *testing.T) {
	...
	// Set and hit.
	c.Set("alice@example.com", "[EMAIL_a3f29c81]")
	token, ok := c.Get("alice@example.com")
	if !ok {
		t.Error("expected hit after Set")
	}
	if token != "[EMAIL_a3f29c81]" {
		t.Errorf("unexpected token: %q", token)
	}

	// Overwrite.
	c.Set("alice@example.com", "[EMAIL_newtoken]")
	token, ok = c.Get("alice@example.com")
	if !ok || token != "[EMAIL_newtoken]" {
...
func TestBboltCacheBasicOperations(t *testing.T) {
	...
	// Set and hit.
	c.Set("bob@corp.io", "[EMAIL_bb3f1c2a]")
	token, ok := c.Get("bob@corp.io")
	if !ok {
		t.Error("expected hit after Set")
	}
	if token != "[EMAIL_bb3f1c2a]" {
		t.Errorf("unexpected token: %q", token)
	}
}
...
func TestBboltCacheSurvivesRestart(t *testing.T) {
	...
	c1.Set("alice@example.com", "[EMAIL_a3f29c81]")
	c1.Set("555-867-5309", "[PHONE_7f4e1b02]")
