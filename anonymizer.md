...
Patterns are organized into packs loaded at startup via the `patternPacks` configuration.
Each pattern carries a confidence score that reflects its false-positive risk:

| Pack    | PII type       | Token prefix    | Example match              | Confidence |
|---------|----------------|-----------------|----------------------------|------------|
| GLOBAL  | Email          | `EMAIL`         | `user@example.com`         | 0.95       |
| GLOBAL  | API key        | `APIKEY`        | `api_key: sk-abc…` (≥20 ch)| 0.90       |
| GLOBAL  | Credit card    | `CREDITCARD`    | `4111 1111 1111 1111`      | 0.85       |
| US      | SSN            | `SSN`           | `123-45-6789`              | 0.85       |
| US      | IPv6 address   | `IPADDRESS`     | `::1`, `2001:db8::1`       | 0.85       |
| US      | Street address | `ADDRESS`       | `123 Main Street`          | 0.75       |
| US      | IPv4 address   | `IPADDRESS`     | `192.168.1.1`              | 0.70       |
| US      | Phone number   | `PHONE`         | `+1-555-123-4567`          | 0.65       |
| DE      | Steuer-ID      | `STEUERID`      | `65929970489`              | 0.90       |
| DE      | SVNR           | `SVNR`          | `12010190A1234`            | 0.85       |
| DE      | KFZ            | `KFZ`           | `B-AB-1234`                | 0.70       |
| SECRETS | SSH key        | `SSHKEY`        | `-----BEGIN RSA PRIVATE KEY-----` | 0.99 |
| SECRETS | JWT            | `JWT`           | `eyJhbGci…`               | 0.95       |
| SECRETS | Bearer token   | `BEARER`        | `Bearer sk-proj-abc…`      | 0.90       |
| SECRETS | DB conn string | `DBCONN`        | `postgres://user:pass@…`   | 0.95       |
| SECRETS | AWS key        | `AWSKEY`        | `AKIAIOSFODNN7EXAMPLE`     | 0.95       |
| SECRETS | GitHub token   | `GHTOKEN`       | `ghp_ABCDEFGHIJ…`         | 0.95       |

If a match's confidence is **at or above** `aiConfidenceThreshold` (default `0.70`), the token
...
For example: `[PII_EMAIL_c160f8cc01234567]`, `[PII_PHONE_7f4e1b0201234567]`.
...
- The bracket notation is chosen to satisfy the **non-retriggering invariant**: no token matches
  any compiled regex pattern from any pack. A violation here would cause the proxy to tokenize
...
token like `[PII_EMAIL_c160f8cc01234567]` frequently arrives split across multiple events:

```
{"type":"text_delta","text":"[PII_EMA"}
{"type":"text_delta","text":"IL_c160f8cc01234567]"}
```
...
4. **Safe flush boundary** (`safeCutPoint`) — calculates how many accumulated bytes can be
   flushed without splitting a partial token. A `tokenSuffixLen` of 50 bytes is retained in
   the accumulator — enough to cover the longest possible token
   (`[PII_CREDITCARD_XXXXXXXXXXXXXXXX]` = 33 chars) plus a 17-byte margin.
