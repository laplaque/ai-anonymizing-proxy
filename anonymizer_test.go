...
// TestLoadPacksWithCustomPattern verifies that custom patterns are loaded and match.
func TestLoadPacksWithCustomPattern(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		AIThreshold:         0.8,
		OllamaMaxConcurrent: 1,
		PatternPacks: []config.PatternPack{
			{Pack: "GLOBAL", Enabled: true},
			{Pack: "CUSTOM", Enabled: true},
		},
		CustomPIIPatterns: []config.CustomPIIPattern{
			{Pack: "CUSTOM", Pattern: "Project\\s+Tempest", Type: "PROJECT", Confidence: 1.0},
		},
	})
	result := a.AnonymizeText("Working on Project Tempest today", "sess-custom")
	if strings.Contains(result, "Project Tempest") {
		t.Errorf("custom pattern not applied: %q", result)
	}
}

// TestLoadPacksWithLiteralCustomPattern verifies literal custom patterns.
func TestLoadPacksWithLiteralCustomPattern(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		...
		CustomPIIPatterns: []config.CustomPIIPattern{
			{Pack: "CUSTOM", Pattern: "op-sunrise", Type: "CODENAME", Confidence: 1.0, Literal: true},
		},
	})
	result := a.AnonymizeText("The project op-sunrise is classified", "sess-literal")
	if strings.Contains(result, "op-sunrise") {
		t.Errorf("literal custom pattern not applied: %q", result)
	}
}

// TestLoadPacksDisabledPackSkipped verifies disabled packs are not loaded.
func TestLoadPacksDisabledPackSkipped(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
	...
	// US phone patterns should not match since US pack is disabled
	phone := fmt.Sprintf("(%s) %s-%s", "555", "123", "4567")
	result := a.AnonymizeText("Call "+phone, "sess-disabled")
	if !strings.Contains(result, phone) {
		t.Errorf("disabled US pack should not match phone, got: %q", result)
	}
}

// TestLoadPacksCustomPatternInvalidRegex verifies invalid custom regex is skipped.
func TestLoadPacksCustomPatternInvalidRegex(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		...
		CustomPIIPatterns: []config.CustomPIIPattern{
			{Pack: "CUSTOM", Pattern: "[invalid", Type: "BAD", Confidence: 0.9},
		},
	})
	// Should not panic; invalid regex silently skipped
	result := a.AnonymizeText("test text", "sess-invalid")
	if result != "test text" {
		t.Errorf("invalid regex should be skipped, text should be unchanged: %q", result)
	}
}

// TestLoadPacksCustomPatternDisabledPack verifies custom patterns in disabled packs are skipped.
func TestLoadPacksCustomPatternDisabledPack(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		...
		CustomPIIPatterns: []config.CustomPIIPattern{
			{Pack: "DISABLED", Pattern: "secret-word", Type: "CUSTOM", Confidence: 1.0, Literal: true},
		},
	})
	result := a.AnonymizeText("the secret-word is here", "sess-disabled-custom")
	if !strings.Contains(result, "secret-word") {
		t.Errorf("custom pattern in disabled pack should not match: %q", result)
	}
}

// TestValidationLayerDropsCandidate verifies that a failed validator drops the match.
func TestValidationLayerDropsCandidate(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
	...
	// Manually add a pattern with a validator that always fails
	a.patterns = append(a.patterns, pattern{
		re:                  regexp.MustCompile(`\bFAILME\b`),
		piiType:             "TEST",
		confidence:          0.99,
		effectiveConfidence: 0.99,
		validate:            func(_ string) bool { return false },
	})
	result := a.AnonymizeText("match FAILME here", "sess-validate")
	if !strings.Contains(result, "FAILME") {
		t.Errorf("failed validation should preserve original text: %q", result)
	}
}

// TestLoadPacksLiteralNoPosDecay verifies literal patterns skip positional decay.
func TestLoadPacksLiteralNoPosDecay(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		AIThreshold:         0.8,
		OllamaMaxConcurrent: 1,
		PackDecayRate:       0.5, // aggressive decay
		PatternPacks: []config.PatternPack{
			{Pack: "FIRST", Enabled: true},
			{Pack: "SECOND", Enabled: true},
			{Pack: "THIRD", Enabled: true},
		},
		CustomPIIPatterns: []config.CustomPIIPattern{
			{Pack: "THIRD", Pattern: "literal-secret", Type: "SECRET", Confidence: 1.0, Literal: true},
		},
	})
	// The literal should match with full confidence despite being in position 3
	result := a.AnonymizeText("the literal-secret is here", "sess-literal-decay")
	if strings.Contains(result, "literal-secret") {
		t.Errorf("literal pattern should match regardless of position decay: %q", result)
	}
}

// TestLoadPacksHighDecayRateClamps verifies multiplier doesn't go below zero.
func TestLoadPacksHighDecayRateClamps(t *testing.T) {
	a := NewWithCacheAndCapacity(Options{
		OllamaEndpoint:      "http://localhost:11434",
		OllamaModel:         "test-model",
		UseAI:               false,
		AIThreshold:         0.0, // accept everything
		OllamaMaxConcurrent: 1,
		PackDecayRate:       1.0, // maximum decay: position 2+ gets 0 multiplier
		PatternPacks: []config.PatternPack{
			{Pack: "FIRST", Enabled: true},
			{Pack: "GLOBAL", Enabled: true}, // position 2: multiplier = 0
		},
	})
	// Email is in GLOBAL (position 2). With decay=1.0, effectiveConfidence = 0.95 * 0.0 = 0
	// Since aiThreshold=0.0, even 0 effective confidence should still match
	result := a.AnonymizeText("test@example.com", "sess-clamp")
	// The email should still be tokenized because effectiveConfidence >= aiThreshold (0.0 >= 0.0)
	if strings.Contains(result, "test@example.com") {
		t.Errorf("pattern should match even with zero effective confidence when threshold is 0: %q", result)
	}
}
