...
PII patterns are organized into packs that can be enabled or disabled via `patternPacks`.
The proxy ships with four built-in packs:

| Pack    | Description                        | Default  |
|---------|------------------------------------|----------|
| GLOBAL  | Email, API keys, credit cards      | enabled  |
| DE      | German Steuer-ID, SVNR, KFZ       | enabled  |
| SECRETS | SSH keys, JWTs, bearer tokens, DB connection strings, AWS keys, GitHub tokens | enabled |
| US      | US phone, SSN, addresses, IPv4/IPv6 | disabled |

Additional packs (FR, NL, FINANCE_EU, HEALTHCARE) are registered but disabled by default.

### Custom patterns

Add custom patterns scoped to a declared pack:
...
Custom type names must not shadow protected built-in types (EMAIL, PHONE, SSN, etc.) and
must be ≤ 15 characters. Invalid patterns are logged and skipped at startup.

### Pack decay rate

`packDecayRate` (default `0.05`) is reserved for future adaptive confidence adjustment.
Valid range: `[0.0, 1.0]`.
...
See [anonymizer.md](anonymizer.md) for the full pattern table with confidence scores.

## Token format

Detected PII is replaced with deterministic tokens of the form `[PII_<TYPE>_<16hex>]` —
e.g. `[PII_EMAIL_c160f8cc01234567]`. The type label gives the LLM semantic context; the
16-hex suffix is the first 16 characters of `md5(original_value)`. See
[anonymizer.md](anonymizer.md) for full details.
