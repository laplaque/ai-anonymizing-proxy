...
e.g. `[PII_EMAIL_c160f8cc]`. The type label gives the LLM semantic context; the 16-hex suffix
is the first 16 characters of `md5(original_value)`. See [anonymizer.md](anonymizer.md) for
full details.

## Pattern packs

All detection patterns are organized into named packs. Packs are declared in `proxy-config.json`
under `patternPacks`. Only enabled packs are loaded at startup.

| Field | Default | Description |
|---|---|---|
| `packDecayRate` | `0.05` | Per-position confidence decay. Must be in [0.0, 1.0]. |
| `patternPacks` | GLOBAL+DE+SECRETS enabled | Ordered list of packs with enable state. |
| `customPIIPatterns` | `[]` | User-defined patterns scoped to a named pack. |

Zero enabled packs is a fatal startup error. See [architecture.md](architecture.md) for details.
