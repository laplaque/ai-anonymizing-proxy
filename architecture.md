...
Tokens are formatted as `[PII_<TYPE>_<16hex>]` — e.g. `[PII_EMAIL_c160f8cc2654c143]` — where `<TYPE>`
is the uppercased PII type name and `<16hex>` is the first 16 hex digits of `md5(original)`.
...
All patterns load from named packs declared in `proxy-config.json`. Pack declaration order
determines precedence via a configurable likelihood multiplier:

```
effectiveConfidence = baseConfidence x (1.0 - (position - 1) x packDecayRate)
```

Default `packDecayRate`: 0.05. Position 1 (first enabled pack) = multiplier 1.0.

A post-regex validation layer runs checksum validators (where present) before candidate merge.
Validation failure drops the candidate and logs a WARN. Literal matches carry their declared
confidence without positional decay.

Built-in packs: `GLOBAL`, `DE`, `SECRETS`, `FR`, `NL`, `FINANCE_EU`, `HEALTHCARE`, `US`.
Defaults enabled: `GLOBAL`, `DE`, `SECRETS`.

![Pack detection pipeline](diagrams/pack-detection-pipeline.png)
...
The 28-byte suffix guard (`tokenSuffixLen`) is retained in the accumulator — enough to cover
the longest possible token (`[PII_CREDITCARD_XXXXXXXXXXXXXXXX]` = 27 chars). Non-delta events (ping,
